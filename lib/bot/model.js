...
export class KarinGroupRecallNotice extends KarinNotice {
   ...
   * @param {Number} time
   */
  constructor (selfId, content, time) {
    const { operator_uid, operator_uin, target_uid, target_uin, message_id, message_seq, group_id, raw_message = '' } = content
    const contact = { scene: 'group', peer: group_id, sub_peer: operator_uid }
    const sender = { operator_uid, operator_uin, target_uid, target_uin }
    super(selfId, 'group_recall', time, contact, sender, content, operator_uid, message_id, message_seq, raw_message, group_id)
  }
