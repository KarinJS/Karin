...
import { segment, common, logger, Cfg, level } from '#Karin'
...
class Bot extends EventEmitter {
  ...
  async sendMsg (id, contact, elements, options = { recallMsg: 0, button: false, retry_count: 1 }) {
    ...
    const { recallMsg, button, retry_count } = options
    ...
    try {
      ...
      /** 取结果 */
      result = await result
      logger.debug(common.logger(self_id, `回复消息结果:${JSON.stringify(result, null, 2)}`))
    } catch (err) {
