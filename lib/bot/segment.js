class Segment {
  ...
  /**
   * 戳一戳
   * @param {string} poke - 类型 见https://github.com/mamoe/mirai/blob/f5eefae7ecee84d18a66afce3f89b89fe1584b78/mirai-core/src/commonMain/kotlin/net.mamoe.mirai/message/data/HummerMessage.kt#L49
   * @param {string} id - ID
   * @param {string} [name] - 表情名
   * @returns {object} 戳一戳消息
   */
  poke (poke, id, name) {
    return { type: 'poke', poke, id, name }
  }
   ...
   * @param {number} [data.style=1] - 按钮样式：0 灰色线框，1 蓝色线框。
    ...
    for (let i of data) {
      ...
      const data = {
        ...
        render_data: {
          ...
          label: i.text || i.link,
          // 按钮样式：0 灰色线框，1 蓝色线框
          style: i.style !== false ? Number(i.style) || 0 : 1,
          // 点击后按钮的上文字
