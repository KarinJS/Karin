...
class Puppeteer extends Renderer {
  ...
  async screenshot (name, data = {}) {
    ...
    /** 页面创建数量到达上限 重启浏览器 */
    if (this.createPageCount >= Cfg.puppeteer.createPageCount) {
...
const renderer = {
   ...
   * @param {boolean} pack - 是否使用segment包裹 默认true
   * @returns {Promise<object>} - 截图数据
   */
  screenshot: async (name, data, pack = true) => {
    let img = await chromium.screenshot(name, data)
    img = 'base64://' + img
    return pack ? segment.image(img) : img
  },
  /**
   * 分页截图
   * @param {string} name - 模板名称
   ...
   */
  screenshots: async (name, data, pack = true) => {
    data.multiPage = true
    let imgs = await chromium.screenshot(name, data) || []
    let ret = []
    for (let img of imgs) {
      img = 'base64://' + img
      ret.push(pack ? segment.image(img) : img)
    }
