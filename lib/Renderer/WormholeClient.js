...
import { URL } from 'url'
import { logger, Renderer } from '#Karin'
...
export default function connect (Config) {
  let heartbeat
  reConnect = undefined
  const { WormholeClient: wsUrl } = Config
  ...
  ws.on('message', function incoming (data) {
    ...
    switch (data.type) {
      case 'msg': {
        const { url, token, WormholeClient } = Config
        ...
        if (data.path) {
          ...
          if (path.extname(filePath) === '.html') {
            ...
            if (!fs.existsSync(html)) {
              return ws.send(JSON.stringify({ type: 'web', state: 'error', error: '文件不存在' }))
            }
