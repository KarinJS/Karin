...
export default class Kritor {
  ...
  init () {
    ...
    this.#server.bindAsync(Cfg.Server.grpc.host, grpc.ServerCredentials.createInsecure(), (err) => {
      if (err) {
        logger.error('grpc服务器启动失败：', err)
      } else {
        logger.info('[服务器][启动成功][grpc]: ', logger.green(`http://${Cfg.Server.grpc.host}`))
      }
    })
    ...
    /** 关闭 gRPC 服务器 */
    Bot.once('exit_grpc', () => this.#server.forceShutdown())
  }
