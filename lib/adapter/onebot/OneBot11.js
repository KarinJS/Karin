 ...
 */
  ...
  async UploadForwardMessage (contact, elements) {
    ...
    const content = this.Elements_OneBot_V11(elements)
    const { name, uid = '', uin = '' } = this.account
    const params = {
      [message_type]: String(peer),
      messages: [{ type: 'node', data: { name, uin: uid || uin, content } }]
    }
    const res_id = await this.SendApi('send_forward_msg', params)
    return { res_id }
  }
  ...
  async GetHistoryMessage (contact, start_message_id, count) {
    let res = (contact.scene === 'group' ? await this.SendApi('get_group_msg_history', { message_id: start_message_id, count, group_id: contact.peer }) : await this.SendApi('get_friend_msg_history', { message_id: start_message_id, count, user_id: contact.peer }))
    let ret = []
    for (let i of res.messages) {
      let message = {
        time: i.time,
        message_id: i.message_id,
        message_seq: i.message_id,
        contact,
        sender: {
          uid: i.sender.user_id,
          uin: i.sender.user_id,
          nick: i.sender.nickname
        },
        elements: this.OneBot_V11_Elements(i.message)
      }
      ret.push(message)
    }
    return ret
