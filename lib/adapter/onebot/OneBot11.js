...
import { logger, Bot, Cfg, common, adapter as KarinAdapter, KarinMessage } from '#Karin'
...
export class OneBot11 extends KarinAdapter {
  ...
  constructor (socket, request) {
    ...
    request && this.#start(socket, request)
  }

  /**
   * 反向ws初始化
   */
  async #start (socket, request) {
    this.socket = socket
    const { host, upgrade, 'x-self-id': id } = request.headers
    const self_id = id
    this.url = `ws://${host + request.url}`
    this.account = {
      uid: self_id,
      uin: self_id,
      name: ''
    ...
    this.logger('info', `[反向WS][onebot11-${upgrade}][${self_id}] ` + logger.green(this.url))
    /** 监听消息事件 */
    ...
    /** 监听断开 */
    this.socket.on('close', () => this.logger('warn', `[反向WS] 连接断开：${this.url}`))
    await this.getSelf()
    this.init = true
  }
  ...
  async start_forward (url) {
    ...
    this.socket.on('open', async () => {
      ...
      logger.info('[正向WS][连接成功][onebot11] ' + logger.green(this.url))
      this.index = 0
      /** 监听消息事件 */
      this.socket.on('message', (bot) => {
        /** 防止空事件 */
      ...
      /** 监听断开 */
      this.socket.on('close', () => this.logger('warn', `[正向WS] 连接断开：${this.url}`))
      await this.getSelf()
      this.init = true
    })
    ...
    this.socket.on('close', async () => {
      this.index++
      logger.warn(`[正向WS][重连次数:${this.index}] 连接断开，将在5秒后重连：${this.url}`)
      /** 停止全部监听 */
      this.socket.removeAllListeners()
      await common.sleep(5000)
      this.start_forward(url)
    })
  }
  ...
  async getSelf () {
    ...
    this.logger('info', `[加载完成][app_name:${this.account.uid}][version:${this.version.version}] ` + logger.green(this.url))
    /** 注册bot */
    Bot.emit('bot', this)
  }
 ...
 */
  ...
  async send_private_msg (user_id, message) {
    let obMessage = this.KarinConvertAdapter(message)
  ...
  async send_group_msg (group_id, message) {
    let obMessages = this.KarinConvertAdapter(message)
  ...
  async GetFriendList () {
    ...
    let friendList = await this.SendApi('get_friend_list')
  ...
  async GetGroupInfo (group_id, no_cache = false) {
     ...
     * }}
     */
    let groupInfo = await this.SendApi('get_group_info', { group_id, no_cache })
    return {
   ...
   */
  async GetGroupList () {
    let groupList = await this.SendApi('get_group_list')
  ...
  async GetGroupMemberInfo (options) {
     ...
     * }}
     */
    let groupMemberInfo = await this.SendApi('get_group_member_info', { group_id, user_id, no_cache: refresh })
    let level = 0
    try {
  ...
  async GetGroupMemberList (options) {
    ...
    let gl = await this.SendApi('get_group_member_list', { group_id })
  ...
  async get_group_honor_info (options) {
     ...
     * }}
     */
    let groupHonor = await this.SendApi('get_group_honor_info', { group_id, type: 'all' })
