...
class OneBot11 {
  ...
  async #start () {
    ...
    logger.mark(`收到新的连接：[OneBot-${this.request.headers.upgrade}][${this.id}] <= => ${this.url}`)
    this.socket.on('message', (e) => {
      logger.debug(`[OneBot-${this.id}][收到上报事件] -> ${e}`)
      e = e.toString().trim()
      if (!e) { return true }
    ...
    this.version = this.get_version_info()
    ...
    return new Promise((resolve) => {
      const timer = setInterval(() => {
        if (this.nickname) {
          ...
          logger.mark(`${logger.violet(`[${this.id}]`)} [${this.adapter.version}-${app_name}(${app_version})] 建立连接成功：${this.url}`)
          clearInterval(timer)
          resolve(true)
        }
      }, 100)
  ...
  #meta_event_event (data) {
    switch (data.meta_event_type) {
      // 心跳
      case 'heartbeat':
        logger.debug(`[OneBot-${this.id}][心跳] -> ${JSON.stringify(data.status, null, 2)}`)
        break
      // 生命周期
      case 'lifecycle':
        logger.mark(`[OneBot-${this.id}][生命周期] -> ${{
          enable: 'OneBot启用',
          disable: 'OneBot停用',
  ...
  async #message_event (data) {
    data = {
       ...
       */
      delete_msg: async message_id => this.delete_msg(message_id || data.message_id),
      /**
       * 快速回复
    ...
    switch (data.message_type) {
      case 'private':
        logger.info(`[OneBot-${this.id}][私聊消息] -> ${data.raw_message}`)
        await loader.deal(data)
        break
      case 'group':
        logger.info(`[OneBot-${this.id}][群消息] -> ${data.raw_message}`)
        await loader.deal(data)
        break
    }
  }
  ...
  #notice_event (data) {
    switch (data.notice_type) {
      case 'group_upload':
        logger.info(`[OneBot-${this.id}][群文件上传] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'group_admin':
        logger.info(`[OneBot-${this.id}][群管理员变动] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'group_decrease':
        logger.info(`[OneBot-${this.id}][群成员减少] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'group_increase':
        logger.info(`[OneBot-${this.id}][群成员增加] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'group_ban':
        logger.info(`[OneBot-${this.id}][群禁言] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'friend_add':
        logger.info(`[OneBot-${this.id}][好友添加] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'group_recall':
        logger.info(`[OneBot-${this.id}][群消息撤回] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'friend_recall':
        logger.info(`[OneBot-${this.id}][好友消息撤回] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'notify':
        switch (data.sub_type) {
          case 'poke':
            logger.info(`[OneBot-${this.id}][群内戳一戳] -> ${JSON.stringify(data, null, 2)}`)
            break
          case 'lucky_king':
            logger.info(`[OneBot-${this.id}][群红包运气王] -> ${JSON.stringify(data, null, 2)}`)
            break
          case 'honor':
            logger.info(`[OneBot-${this.id}][群成员荣誉变更] -> ${JSON.stringify(data, null, 2)}`)
            break
        }
        break
    }
  ...
  #request_event (data) {
    switch (data.request_type) {
      case 'friend':
        logger.info(`[OneBot-${this.id}][加好友请求] -> ${JSON.stringify(data, null, 2)}`)
        break
      case 'group':
        logger.info(`[OneBot-${this.id}][加群请求/邀请] -> ${JSON.stringify(data, null, 2)}`)
        break
    }
  }
 ...
 */
  ...
  async SendApi (action, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on(echo, (data) => (data.status === 'ok' ? resolve(data.data) : reject(data)))
    })
  }
}
export default OneBot11
