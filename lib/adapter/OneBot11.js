...
class OneBot11 {
  ...
  async #start () {
    ...
    logger.mark(this.#log(`收到新的连接：[OneBot-${this.request.headers.upgrade}][${this.id}] <==> ${this.url}`))
  ...
  #meta_event_event (data) {
    ...
    switch (data.meta_event_type) {
      case 'heartbeat':
        logger.debug(this.#log(`[心跳] -> ${JSON.stringify(data.status, null, 2)}`))
        break
      case 'lifecycle':
  ...
  async #message_event (data) {
    ...
    switch (data.message_type) {
      case 'private':
        ...
        break
      case 'group':
        logger.info(this.#log(`[群聊:${data.group_id}-${data.user_id}(${data.sender.nickname || ''})] ${data.raw_message}`))
        break
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on(echo, (data) => {
        if (data.status === 'ok') {
          ...
          logger.error(this.#log(`${action}: ${JSON.stringify(data, null, 2)}`))
          reject(data)
        }
      })
    })
