...
import { logger, handler, button, common, listener } from '../index.js'
...
class Loader {
  ...
  async load () {
    ...
    listener.once('plugin.loader', () => {
      for (const v of this.watchList) {
        v.type === 'folder' ? this.watchDir(v.dir) : this.watch(v.dir, v.name)
      }
    })
    ...
    for (const val of plugins) {
      ...
      if (fs.existsSync(`${_path}/package.json`)) {
        ...
        if (fs.existsSync(`${_path}/index.js`)) {
          ...
          /** dev监听index.js */
          if (process.argv[2]?.includes('dev')) this.hotUpdate('file', dir, 'index.js')
        }
        /** apps存在 */
  ...
  getApps (dir) {
    ...
    if (dir.endsWith('apps')) {
      /** dev监听apps热更 */
      if (process.argv[2]?.includes('dev')) this.hotUpdate('folder', dir)
    } else {
      /** 监听文件夹热更新 */
      this.hotUpdate('folder', dir)
    }
    return App
  }
  ...
  async createdApp (dir, name, isOrderBy = false) {
    try {
      ...
      if (/Cannot find package '(.+?)'/.exec(error)?.[1]) {
        logger.debug(error.stack || error.message || JSON.stringify(error))
   ...
   * @param {string} [name] 文件名称 只有在type为folder时才需要
   */
  async hotUpdate (type, dir, name) {
    type === 'folder' ? this.watchList.push({ type, dir }) : this.watchList.push({ type, dir, name })
  }
