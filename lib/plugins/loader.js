  ...
  getPlugins () {
    ...
    for (let val of plugins) {
      ...
      if (fs.existsSync(`${_path}/index.js`)) {
        ...
        /** apps存在 */
        if (fs.existsSync(`${_path}/apps`) && fs.statSync(`${_path}/apps`).isDirectory()) dir = `${dir}/apps`
      }
  ...
  async createdApp (dir, name, isOrderBy = false) {
    try {
      ...
      if (error.code === 'ERR_MODULE_NOT_FOUND') {
        ...
        logger.mark('--------插件载入错误--------')
        logger.mark(`错误: ${name} 缺少必要的依赖项: ${logger.red(pack)}`)
        logger.mark(`操作：请尝试在命令中断中执行 ${logger.red('pnpm i -P')} 命令安装依赖项`)
        logger.warn('提示：如安装后仍未解决，可选择以下方案')
        logger.warn(`      1.手工安装依赖: ${logger.red('pnpm i ' + pack)}`)
        logger.warn(`      2.联系插件作者：联系插件作者将 ${logger.red(pack)} 依赖项添加至插件的package.json文件中的dependencies字段中`)
        logger.mark('-----------------------------')
      } else {
