...
class Update {
  ...
  getPlugins () {
    ...
    files.forEach(file => {
      ...
      if (!fs.existsSync(`${this.dir}/${file.name}/package.json`)) return
      list.push(file.name)
    })
    return list
  }
   ...
   */
  async update (path, cmd = 'git pull --rebase', time = 120) {
    /** 检查一下路径是否存在 */
    ...
    /** 设置超时时间 */
    let timer = setTimeout(() => { return { status: 'failed', data: '执行超时' } }, time * 1000)

    const options = { env: process.env, cwd: path, encoding: 'utf-8' }
    /** 记录当前短哈希 */
    let hash = await this.getHash(path)

    /** 执行更新 */
    const { status, error } = await exec(cmd, false, options)

    /** 执行成功 */
    if (status === 'ok') {
      clearTimeout(timer)
      /** 再次获取短哈希 查看是否有更新 */
      let newHash = await this.getHash(path)
      if (hash === newHash) {
