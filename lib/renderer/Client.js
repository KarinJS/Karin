...
export default class RenderClient extends RenderBase {
  constructor (url) {
    ...
    this.index = 0
    this.retry = 0
    this.reg = new RegExp(`(${process.cwd()}|${process.cwd().replace(/\\/g, '/')})`, 'g')
  }
  ...
  async message (data) {
    ...
    switch (data.action) {
      /** 静态文件 */
      case 'static': {
        const file = fs.readFileSync('.' + decodeURIComponent(data.params.file))
        const params = {
          echo: data.echo,
          action: 'static',
          status: 'ok',
  ...
  async render (options) {
    ...
    logger.debug(`[渲染器:${this.id}:${this.index}] ws发送消息：${req}`)
    this.ws.send(req)
