...
import { URL } from 'url'
import { logger, Renderer, Cfg } from '#Karin'
    ...
    switch (data.type) {
      ...
      case 'web':
        if (data.path) {
          ...
          if (query.html) {
            ws.send(JSON.stringify({ type: 'web', command: 'redirect', path: filePath, target: query.html.startsWith('/') ? query.html.slice(1) : query.html, echo: echo }))
            return
          }
          ...
          if (!list.some(ext => path.extname(filePath).endsWith(ext))) {
            ...
            ws.send(JSON.stringify({ type: 'web', state: 'error', error: '非资源文件', echo: echo }))
            return
          }
          ...
          if (path.extname(filePath) === '.html') {
            ...
            if (!fs.existsSync(html)) {
              return ws.send(JSON.stringify({ type: 'web', state: 'error', error: '文件不存在', echo: echo }))
            }
