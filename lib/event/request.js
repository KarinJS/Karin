 ...
 */
  async deal (e) {
    ...
    if (e.contact.scene === 'friend') {
      ...
      logger.info(common.logger(e.self_id, `私聊请求事件：[${e.user_id}(${e.sender.nick || ''})] ${e.raw_message}`))
    } else if (e.contact.scene === 'group') {
      e.isGroup = true
      e.logText = `[Group:${e.group_id}-${e.user_id}(${e.sender.nick || ''})]`
      logger.info(common.logger(e.self_id, `群请求事件：[${e.group_id}-${e.user_id}(${e.sender.nick || ''})] ${e.raw_message}`))
    } else {
      logger.info(common.logger(e.self_id, `未知来源请求事件：${JSON.stringify(e)}`))
    }
