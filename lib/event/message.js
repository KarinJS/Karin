...
export default class Message extends Event {
    ...
    for (const app of loader.Apps) {
      ...
      for (const v of app.rule) {
        ...
        if (v.reg.test(this.e.msg)) {
          ...
          this.GroupMsgPrint && v.log(this.e.self_id, `${this.e.logFnc}${this.e.logText} ${lodash.truncate(this.e.msg, { length: 80 })}`)
          ...
          try {
            ...
            this.GroupMsgPrint && v.log(this.e.self_id, `${this.e.logFnc} ${lodash.truncate(this.e.msg, { length: 80 })} 处理完成 ${logger.green(Date.now() - start + 'ms')}`)
  ...
  dealMsg () {
    const logs = []
    for (let val of this.e.elements) {
      switch (val.type) {
        case 'text': {
    ...
    /** 前缀处理 */
    Review.ailas(this.e, this.config)

    /** 星铁命令前缀 */
    const srReg = /^#?(\*|星铁|星轨|穹轨|星穹|崩铁|星穹铁道|崩坏星穹铁道|铁道)+/
    if (srReg.test(this.e.msg)) {
      this.e.game = 'sr'
      this.e.msg = this.e.msg.replace(srReg, '#星铁')
    }
   ...
   */
  async context () {
    let key = this.e.isGroup ? `${this.e.group_id}.${this.e.user_id}` : this.e.user_id
    ...
    if (App) {
      ...
      let start = Date.now()
      plugin.e = this.e
      let res = plugin[fnc] && plugin[fnc](this.e)
      if (util.types.isPromise(res)) res = await res
