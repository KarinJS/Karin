...
import type { KarinScreenshotOptions, RenderResult, Snapka, SnapkaResult } from './types'

/** 渲染函数类型 */
export interface Render {
  <T extends KarinScreenshotOptions> (options: T): Promise<RenderResult<T>>
 ...
 * @param id 指定渲染器ID
 */
export const callRender = async <T extends KarinScreenshotOptions> (options: T, id?: string) => {
...
class RenderCache {
  ...
  /**
     * 调用标准渲染器
     */
  async render<T extends KarinScreenshotOptions> (options: T, id?: string): Promise<RenderResult<T>> {
