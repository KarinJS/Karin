...
export class AdapterOneBot<T extends OneBotType = OneBotType> extends AdapterBase {
  ...
  constructor (core: T) {
    ...
    this.core = core
    this.converter = new OneBotConverter(this)
    this.createEvent = new CreateOneBotEvent(this)
  }
  ...
  setAdapterInfo () {
    this.adapter.platform = 'qq'
    this.adapter.name = this.core.protocol.name
    this.adapter.version = this.core.protocol.version
  ...
  async sendApi<T extends keyof OneBotApi> (
    ...
    return this.core.sendApi(action, params, timeout)
  }
  ...
  AdapterConvertKarin (data: Array<OneBotMessage>) {
    return this.converter.adapterConvertKarin(data)
  }
  ...
  KarinConvertAdapter (data: Array<SendElement>) {
    return this.converter.karinConvertAdapter(data)
  }
  ...
  async sendMsg (
    ...
    try {
      ...
      if (contact.scene === 'group') {
        res = await this.core.sendGroupMsg(Number(contact.peer), message)
      } else if (contact.scene === 'groupTemp') {
        res = await this.core.sendPrivateMsg(Number(contact.subPeer), message)
      } else {
        res = await this.core.sendPrivateMsg(Number(contact.peer), message)
      }
  ...
  async sendLongMsg (contact: Contact, resId: string) {
    const result = await (async () => {
      if (contact.scene === 'group') {
        return await this.core.sendGroupMsg(
          Number(contact.peer),
      ...
      return await this.core.sendPrivateMsg(
        id,
        [{ type: OneBotMessageType.Forward, data: { id: resId } }]
      )
    })()
  ...
  async recallMsg (_: Contact, messageId: string) {
    await this.core.deleteMsg(Number(messageId))
  }
  ...
  async getMsg (_contact: Contact | string, messageId?: string) {
    ...
    const result = await this.core.getMsg(Number(targetId))
  ...
  async getHistoryMsg (contact: Contact, startMsgId: string | number, count: number) {
    const result = await (async (): Promise<ReturnType<OneBotCore['getGroupMsgHistory']> | ReturnType<OneBotCore['nc_getFriendMsgHistory']>> => {
      ...
      if (typeof startMsgId === 'number') {
        ...
        return this.core.getGroupMsgHistory(targetId, startMsgId, count)
      }
      ...
      if (this.adapter.name === 'Lagrange.OneBot') {
        if (contact.scene === 'group') {
          return this.core.lgl_getGroupMsgHistory(targetId, Number(startMsgId), count)
        } else if (contact.scene === 'friend') {
          return this.core.lgl_getFriendMsgHistory(targetId, Number(startMsgId), count)
        } else {
      ...
      if (contact.scene === 'group') {
        return this.core.getGroupMsgHistory(targetId, seq.message_seq, count)
      } else if (contact.scene === 'friend') {
        return this.core.nc_getFriendMsgHistory(targetId, seq.message_seq, count)
      } else {
        throw new Error(`不支持的消息环境:${contact.scene}`)
      }
    })()
  ...
  async sendLike (targetId: string, count: number) {
    await this.core.sendLike(Number(targetId), count)
  }
  ...
  async groupKickMember (groupId: string, targetId: string, rejectAddRequest?: boolean, _?: string) {
    await this.core.setGroupKick(Number(groupId), Number(targetId), rejectAddRequest)
  }
  ...
  async setGroupMute (groupId: string, targetId: string, duration: number) {
    await this.core.setGroupBan(Number(groupId), Number(targetId), duration)
  }
  ...
  async setGroupAllMute (groupId: string, isBan: boolean) {
    await this.core.setGroupWholeBan(Number(groupId), isBan)
  }
  ...
  async setGroupAdmin (groupId: string, targetId: string, isAdmin: boolean) {
    await this.core.setGroupAdmin(Number(groupId), Number(targetId), isAdmin)
  }
  ...
  async setGroupMemberCard (groupId: string, targetId: string, card: string) {
    await this.core.setGroupCard(Number(groupId), Number(targetId), card)
  }
  ...
  async setGroupName (groupId: string, groupName: string) {
    await this.core.setGroupName(Number(groupId), groupName)
  }
  ...
  async setGroupQuit (groupId: string, isDismiss: boolean) {
    await this.core.setGroupLeave(Number(groupId), isDismiss)
  }
  ...
  async setGroupMemberTitle (groupId: string, targetId: string, title: string) {
    await this.core.setGroupSpecialTitle(Number(groupId), Number(targetId), title)
  }
  ...
  async getStrangerInfo (targetId: string) {
    const result = await this.core.getStrangerInfo(Number(targetId[0]), true)
    return {
      ...result,
      userId: targetId,
      user_id: targetId,
  ...
  async getFriendList (_?: boolean) {
    const friendList = await this.core.getFriendList()
  ...
  async getGroupInfo (groupId: string, noCache?: boolean) {
    const info = await this.core.getGroupInfo(Number(groupId), noCache)
  ...
  async getGroupList (_refresh?: boolean) {
    ...
    const groupList = await this.core.getGroupList()
  ...
  async getGroupMemberInfo (groupId: string, targetId: string, refresh?: boolean) {
    ...
    const info = await this.core.getGroupMemberInfo(Number(groupId), userId, refresh)
    return {
      ...info,
      userId: targetId,
      uid: targetId,
  ...
  async getGroupMemberList (groupId: string, refresh?: boolean) {
    const list = await this.core.getGroupMemberList(Number(groupId), refresh)
    return list.map(v => {
  ...
  async getGroupHonor (groupId: string) {
    const groupHonor = await this.core.getGroupHonorInfo(Number(groupId), 'all')
  ...
  async setMsgReaction (contact: Contact, messageId: string, faceId: number, isSet: boolean) {
    if (this.adapter.name === 'Lagrange.OneBot') {
      await this.core.lgl_setGroupReaction(+contact.peer, +messageId, faceId + '', isSet)
      return
    }

    if (this.adapter.name === 'NapCat.Onebot') {
      await this.core.nc_setMsgEmojiLike(+messageId, faceId + '', isSet)
      return
    }
   ...
   */
  async getVersion () {
    const res = await this.core.getVersionInfo()
    return {
      name: res.app_name,
  ...
  async getGroupHighlights (groupId: string, _: number, __: number) {
    ...
    const res = await this.core.getEssenceMsgList(Number(groupId))
    for (const v of res) {
  ...
  async uploadFile (contact: Contact, file: string, name: string, folder?: string) {
    if (contact.scene === 'group') {
      await this.core.uploadGroupFile(+contact.peer, file, name, folder)
      return
    }

    await this.core.uploadPrivateFile(+contact.peer, file, name)
  }
  ...
  async setGgroupHighlights (_: string, messageId: string, create: boolean) {
    if (create) {
      await this.core.setEssenceMsg(+messageId)
    } else {
      await this.core.deleteEssenceMsg(+messageId)
    }
  }
  ...
  async setFriendApplyResult (requestId: string, isApprove: boolean, remark?: string) {
    await this.core.setFriendAddRequest(requestId, isApprove, remark)
  }
  ...
  async setGroupApplyResult (requestId: string, isApprove: boolean, denyReason?: string) {
    await this.core.setGroupAddRequest(requestId, 'add', isApprove, denyReason)
  }
  ...
  async setInvitedJoinGroupResult (requestId: string, isApprove: boolean) {
    await this.core.setGroupAddRequest(requestId, 'invite', isApprove)
  }
  ...
  async sendForwardMsg (contact: Contact, elements: NodeElement[], options?: ForwardOptions) {
    const result = await (() => {
      if (contact.scene === 'group') {
        return this.core.sendGroupForwardMsg(
          Number(contact.peer),
      ...
      if (contact.scene === 'friend') {
        return this.core.sendPrivateForwardMsg(
          Number(contact.peer),
          this.forwardKarinConvertAdapter(elements, options)
        )
      }
  ...
  async getFileUrl (contact: Contact, fid: string) {
    if (this.adapter.name === 'NapCat.Onebot') {
      const { url } = await this.core.nc_getFile(fid)
      return url
    }

    if (contact.scene === 'group') {
      const { url } = await this.core.getGroupFileUrl(+contact.peer, fid)
      return url
    }

    if (contact.scene === 'friend') {
      const { url } = await this.core.getPrivateFileUrl(+contact.peer, fid)
      return url
    }
  ...
  async getGroupFileSystemInfo (groupId: string) {
    const result = await this.core.getGroupFileSystemInfo(+groupId)
    return {
      ...result,
  ...
  async getGroupFileList (groupId: string, folderId?: string) {
    const result = typeof folderId !== 'string'
      ? await this.core.getGroupRootFiles(+groupId)
      : await this.core.getGroupFilesByFolder(+groupId, folderId)
  ...
  async getCookies (domain: string) {
    const result = await this.core.getCookies(domain)
  ...
  async getCredentials (domain: string) {
    const result = await this.core.getCredentials(domain)
   ...
   */
  async getCSRFToken () {
    const result = await this.core.getCsrfToken()
  ...
  async setAvatar (file: string) {
    await this.core.setQqAvatar(file)
  }
  ...
  async getAiCharacters () {
    const result = await this.core.getAiCharacters()
    return result
  }
  ...
  async sendAiCharacter (groupId: string, character: string, text: string) {
    const result = await this.core.sendGroupAiRecord(+groupId, character, text)
   ...
   */
  async getRkey () {
    const result = await this.core.getRkey()
    return result.rkeys
  }
}
