...
describe('cache/LRU', () => {
  ...
  it('peek removes expired and returns undefined', () => {
    const lru = new LRUCache<string, number>(10)
    const base = Date.now()
    const spy = vi.spyOn(Date, 'now').mockReturnValue(base)
    lru.set('e', 1, 10)
    spy.mockReturnValue(base + 20)
    expect(lru.peek('e')).toBeUndefined()
    spy.mockRestore()
  })

  it('has returns false when expired and cleans up', () => {
    const lru = new LRUCache<string, number>(10)
    const base = Date.now()
    const spy = vi.spyOn(Date, 'now').mockReturnValue(base)
    lru.set('x', 1, 10)
  ...
  it('size triggers cleanup of expired entries', () => {
    const lru = new LRUCache<string, number>(10)
    const base = Date.now()
    const spy = vi.spyOn(Date, 'now').mockReturnValue(base)
    lru.set('a', 1, 10)
