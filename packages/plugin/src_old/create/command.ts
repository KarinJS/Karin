...
export const command = <T extends EventTypes = keyof MessageEventMap> (
  ...
  pluginCache.register.command(result as unknown as CreateCommand)
  return result
}
...
export class CreateCommand<T extends EventTypes = EventTypes> extends BuilderBase {
  ...
  constructor (
    ...
    this.#options = CreateCommand.options(options, this.defaultAppName)
  ...
  static callback<T extends EventTypes> (
    ...
    if (typeof fnc === 'function') {
      return async (ctx: EventsToUnion<T>, next: () => void) => {
        // 异步追踪热点缓存（不阻塞命令执行）
        if (command) {
          pluginCache.hotCache.track(ctx.msg, command as unknown as CreateCommand).catch(() => { })
        }
  ...
  setOptions (options: Options<T> | StringOptions<T>) {
    this.#rawOptions = options
    const opt = CreateCommand.options(options, this.defaultAppName)
