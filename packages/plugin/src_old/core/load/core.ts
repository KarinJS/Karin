...
import { registerModule } from '../../create/class'
...
export class PluginsLoader {
  ...
  /**
   * 获取 package.karin 主文件
   * @param pkg - package.json 内容对象
   * @param abs - 插件包绝对路径
   * @returns 主文件绝对路径或 null
   */
  getKarinMain (pkg: Package, abs: string): string | null {
    const main = isDev() ? pkg.karin?.main : pkg.main
    return main ? formatPath(path.join(abs, main)) : null
  }
  ...
  async loadMain (meta: PackageMetaInfoCache): Promise<void> {
    const info = pluginCache.package.get(meta.name)
    if (!info) {
  ...
  async importEntryFile (meta: PackageMetaInfoCache, file: string): Promise<Record<string, unknown> | null> {
    ...
    const result = await imports<Record<string, unknown>>(file).catch((error) => {
      ...
      /** 记录import错误 */
      pluginCache.missingDeps.add(meta.name, file, error)
      return null
    })
  ...
  async loadEntry (meta: PackageMetaInfoCache): Promise<void> {
    const files = pluginCache.package.getFilesByPackageName(meta.name)
    if (files.length === 0) {
  ...
  async setup (
    ...
    pluginCache.public.set(meta.name, options.public)
  ...
  async setupV1 (
    ...
    await this.createDirs(meta.name, options.files)
    pluginCache.public.set(meta.name, options.public)
    pluginEnv.createV1(meta.name, options.env)
  }
  ...
  addCache (meta: PackageMetaInfoCache, type: PluginsTypes, main: string | null, entry: string[]): void {
    ...
    entry.forEach((file) => {
      pluginCache.package.addFile(meta.name, file)
    })
  }
}
