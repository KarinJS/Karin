...
export class PluginsLoaderGit extends PluginsLoader {
  /**
   * 初始化插件加载器
   */
  async init () {
    /** 扫描列表 */
    const { git } = await packageFinder.getPluginsPackages()
    /** 设置插件列表缓存 */
    pluginCache.list.set('git', git)
    await Promise.all(git.map((pkg) => this.load(pkg)))
    return this
  }
  ...
  private async load (meta: PackageMetaInfoCache) {
    this.logger.debug(`📂 开始加载 Git 插件 ${global.logger.cyan(meta.name)}`)
    ...
    this.logger.debug(`✅ Git 插件 ${global.logger.cyan(meta.name)} 加载完成`)
    return this
  }
  ...
  private async tryLoadV2 (
    ...
    this.addCache(meta, 'git', this.getMain(pkg, meta.abs), entry)
  ...
  private async tryLoadV1 (
    ...
    this.addCache(meta, 'git', this.getKarinMain(pkg, meta.abs), entry)
