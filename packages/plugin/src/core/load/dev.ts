...
import { LoadStatus, PluginsLoader, PackageMetaInfoCache } from './core'
...
export class PluginsLoaderDev extends PluginsLoader {
  ...
  private async tryLoadV1 (
    ...
    await this.setup(meta, { files: pkg.karin.files, public: pkg.karin.static, env: pkg.karin.env })
    ...
    this.addCache(meta, 'dev', this.getKarinMain(pkg, meta.abs), entry)
