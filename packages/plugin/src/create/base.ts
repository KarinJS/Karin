...
import type { PluginTypes } from '../cache/default'
...
export abstract class BuilderBase {
  ...
  get file (): PluginCacheFile {
    ...
    return {
      ...
      get relPath () {
        ...
        if (!relPath) {
          relPath = pluginCache.package.getAbsPath(_this.packageName).replaceAll('\\', '/')
  ...
  get packageName (): string {
    ...
    return pluginCache.package.getPackageNameByFile(callerDir) ||
      `${this.type}:unknown:${this.file.basename}`
  }
