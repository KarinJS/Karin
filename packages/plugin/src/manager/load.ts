...
import { loadClass } from '../decorators/class'
import { plguinManager } from '../manager'
import { createPluginDir, importModule, satisfies } from '@karinjs/utils'
...
export const init = async () => {
  if (isInit) return
  await plguinManager.manager.getPluginDetails()
  const list = await plguinManager.manager.getPlugins()
  ...
  await new Promise((resolve) => {
    let lastLength = plguinManager.manager.cache.command.length
    const timer = setInterval(() => {
      if (plguinManager.manager.cache.command.length === lastLength) {
        clearInterval(timer)
        resolve(true)
      }

      lastLength = plguinManager.manager.cache.command.length
    }, 500)
  })

  plguinManager.register.sort()
  isInit = true

  logger.info('插件加载完成')

  logger.info(`${logger.chalk.magentaBright('plugin')}: ${plguinManager.manager.cache.plugins.length}`)
  Object.keys(plguinManager.manager.cache.count).forEach((v) => {
    if (v === 'handler') {
      const { key, fnc } = plguinManager.manager.cache.count.handler
      logger.info(`${logger.chalk.magentaBright(v + '.key')} ${key}`)
      logger.info(`${logger.chalk.magentaBright(v + '.fnc')} ${fnc}`)
      return
    }
    logger.info(`${logger.chalk.magentaBright(v)}: ${plguinManager.manager.cache.count[v as keyof typeof plguinManager.manager.cache.count]}`)
  })
...
export const load = async (pluginName: string) => {
  const pkg = plguinManager.manager.getPluginPackageDetail(pluginName)
  ...
  /** 加载入口文件 */
  if (pkg?.data) {
    const main = plguinManager.manager.getMain(pkg.data, pkg.dir)
  ...
  /** 获取静态资源目录 */
  getStaticDir(pkg!)
  plguinManager.manager.cache.count.pkg++
}
...
const getStaticDir = (pkg: PluginCacheKeyPkg) => {
  ...
  if (!pkg.data.karin?.static) {
    plguinManager.manager.cache.static.push(path.resolve(pkg.dir, 'resource'))
    plguinManager.manager.cache.static.push(path.resolve(pkg.dir, 'resources'))
    return
  }

  /** 静态资源目录处理 */
  if (typeof pkg.data.karin?.static === 'string') {
    return plguinManager.manager.cache.static.push(path.resolve(pkg.dir, pkg.data.karin.static))
  }

  if (Array.isArray(pkg.data.karin?.static)) {
    return plguinManager.manager.cache.static.push(...pkg.data.karin.static?.map(file => path.resolve(pkg.dir, file)))
  }
}
