...
import { PluginInstaller, spawnProcess } from '../plugins/core'

import type { RequestHandler, Response } from 'express'
import type { TaskEntity, TaskType } from '@karinjs/db'
...
export const manageDependenciesRouter: RequestHandler<null, null, DependenciesManage> = async (req, res) => {
  ...
  if (type === 'add') {
     ...
     * ```
     */
    await addDependencies(res, data, req.ip!)
    return
  }
  ...
  if (type === 'upgrade') {
     ...
     * ```
     */
    await installDependencies(res, data, req.ip!)
    return
  }
  ...
  if (type === 'remove') {
     ...
     * ```
     */
    await removeDependencies(res, data, req.ip!)
    return
  }

  PluginInstaller.handleReturn(res, false, '无效请求：不支持的操作类型')
}
...
const installDependencies = async (
  ...
  try {
    ...
    return PluginInstaller.handleReturn(res, true, '安装任务已创建', id)
  } catch (error) {
    logger.error('[installDependencies]', error)
    logger.mark(`安装依赖 ${logger.red(dependencies.join(' '))} 失败`)
    return PluginInstaller.handleReturn(res, false, `安装失败: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
const removeDependencies = async (
  ...
  try {
    ...
    return PluginInstaller.handleReturn(res, true, '删除任务已创建', id)
  } catch (error) {
    logger.error('[removeDependencies]', error)
    logger.mark(`删除依赖 ${logger.red(dependencies.join(' '))} 失败`)
    return PluginInstaller.handleReturn(res, false, `删除失败: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
const addDependencies = async (
  ...
  try {
    if (!dependencies.name || !dependencies.location) {
      return PluginInstaller.handleReturn(res, false, '无效请求：缺少必要参数')
    }
    ...
    return PluginInstaller.handleReturn(res, true, '添加任务已创建', id)
  } catch (error) {
    logger.error('[addDependencies]', error)
    logger.mark(`新增依赖 ${logger.red(dependencies.name)} 失败`)
    return PluginInstaller.handleReturn(res, false, `添加失败: ${error instanceof Error ? error.message : String(error)}`)
  }
}
