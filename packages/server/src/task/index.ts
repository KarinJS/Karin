...
import type { TaskStatus } from '@karinjs/db'
...
export const taskRunRouter: RequestHandler<null, null, null, { id: string, lastIndex?: string }> = async (req, res) => {
  ...
  const task = await webuiDB.get(id)
  if (!task) {
  ...
  /** 检查任务是否存在 */
  if (await webuiDB.exists(task.type, task.target, ['running'])) {
...
export const taskLogsRouter: RequestHandler<{ id: string }> = async (req, res) => {
  ...
  try {
    ...
    if (task.status === 'running' || task.status === 'pending') {
      const logs = await webuiDB.logs(id)
    ...
    timer = setInterval(async () => {
      try {
        const info = await webuiDB.get(id)
