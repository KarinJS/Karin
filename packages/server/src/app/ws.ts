...
export const emitEvent = (
  ...
  emitter.emit(key, socket, request, call)
}
...
wss.on('connection', (socket, request) => {
  ...
  if (
    ...
    socket.on('close', (code, reason) => {
      emitter.emit(WS_CLOSE_ONEBOT, socket, request, code, reason)
    })
  ...
  if (request.url === '/puppeteer' || request.url === '/snapka') {
    ...
    socket.once('close', (code, reason) => {
      emitter.emit(WS_CLOSE_PUPPETEER, socket, request, code, reason)
    })
  ...
  socket.on('close', (code, reason) => {
    emitter.emit(WS_CLOSE, socket, request, code, reason)
  })
})
