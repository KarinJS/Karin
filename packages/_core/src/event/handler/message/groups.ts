...
import type { GroupMessage, GroupTempMessage, GuildMessage } from '../../types/message'
...
export const groupHandler = async (ctx: GroupMessage) => {
  ...
  const eventConfig = getGroupCfg(ctx.groupId, ctx.selfId)
  const isPrint = utils.filter.groupLogEnabled(ctx, systemConfig)

  /** 定义群聊特定钩子函数 */
  const hook = async (ctx: GroupMessage) => await hooksMessageEmit.group(ctx)
...
export const groupTempHandler = async (ctx: GroupTempMessage) => {
  ...
  /** 定义群聊临时消息特定钩子函数 */
  const hook = async (ctx: GroupTempMessage) => await hooksMessageEmit.groupTemp(ctx)
...
export const guildHandler = async (ctx: GuildMessage) => {
  /** 获取配置 */
  const systemConfig = cfg()
  const eventConfig = getGuildCfg(ctx.guildId, ctx.channelId, ctx.selfId)
  const isPrint = utils.filter.guildLogEnabled(ctx, systemConfig)

  /** 定义频道特定钩子函数 */
  const hook = async (ctx: GuildMessage) => await hooksMessageEmit.guild(ctx)
...
const _dispatch = ({
  ...
  eventConfig: ReturnType<typeof getGuildCfg | typeof getGroupCfg>,
