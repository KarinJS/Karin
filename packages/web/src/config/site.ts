...
export const initSiteConfig = async (isRefresh = false) => {
  ...
  // 查找插件管理菜单项
  const pluginNavItem = siteConfig.navItems.find(item => item.href === '/plugins-dashboard')

  if (pluginNavItem && plugins.length > 0) {
    // 将插件列表转换为符合NavItem.children格式的数组，并按hasConfig排序
    pluginNavItem.children = plugins
      .map(plugin => ({
        id: plugin.id,
        href: plugin.href,
        label: plugin.label,
