...
import { initDefaultValues } from './values'
 ...
 *
 */
export const Test: React.FC<GetConfigResponse> = ({ options, info }) => {
  ...
  return (
    <Form onSubmit={onSubmit} className='flex flex-col gap-4 p-4'>
      <div className='space-y-4'>
        {options.map((option) => {
          return <RenderComponent config={option} register={register} control={control} />
        })}
      </div>
    </Form>
  )
