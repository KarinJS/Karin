...
type SystemEnvFormData = {
  ...
  PM2_RESTART: boolean
  TSX_WATCH: boolean
  LOG_LEVEL: 'all' | 'trace' | 'debug' | 'mark' | 'info' | 'warn' | 'error' | 'fatal' | 'off'
  LOG_DAYS_TO_KEEP: number
  LOG_MAX_LOG_SIZE: number
  LOG_FNC_COLOR: string
  LOG_MAX_CONNECTIONS: number
  FFMPEG_PATH: string
  FFPROBE_PATH: string
  FFPLAY_PATH: string
  RUNTIME: 'node' | 'tsx' | 'pm2'
  NODE_ENV: 'development' | 'production' | 'test'
}

export const getSystemEnvComponent = (
  data: SystemEnvFormData,
  formRef: React.RefObject<HTMLFormElement | null>
) => {
  const methods = useForm<SystemEnvFormData>({
    defaultValues: data
  })

  const onSubmit = (formData: SystemEnvFormData) => {
    console.log('系统环境变量提交:', formData)
  }
          ...
          <div className='p-4 rounded-lg border border-gray-200'>
            ...
            <div className='grid md:grid-cols-2 gap-4'>
              ...
              <Input
                ...
                size='sm'
                {...methods.register('HTTP_AUTH_KEY')}
              />
              <Input
                className='p-1 rounded-lg w-full'
                color='primary'
                ...
                size='sm'
                {...methods.register('WS_SERVER_AUTH_KEY')}
              />
            </div>
          </div>
          ...
          <div className='p-4 rounded-lg border border-gray-200'>
            ...
            <div className='space-y-4'>
              ...
              <div className='flex flex-col gap-2'>
                ...
                <div className='flex'>
                  <ColorPicker
                    color={methods.watch('LOG_FNC_COLOR')}
                    onChange={(color) => methods.setValue('LOG_FNC_COLOR', color)}
                  />
                </div>
              </div>
              <Controller
              ...
              <div className='grid md:grid-cols-3 grid-cols-1 gap-4'>
                ...
                <NumberInput
                  ...
                  isRequired
                  min={1}
                  {...methods.register('LOG_DAYS_TO_KEEP')}
                />
                {/* @ts-ignore */}
                <NumberInput
                  className='p-1 rounded-lg w-full'
                  ...
                  isRequired
                  min={0}
                  {...methods.register('LOG_MAX_LOG_SIZE')}
                />
                {/* @ts-ignore */}
                <NumberInput
                  className='p-1 rounded-lg w-full'
                  ...
                  min={1}
                  max={10}
                  {...methods.register('LOG_MAX_CONNECTIONS')}
                />
              </div>
            </div>
          </div>
          ...
          <div className='p-4 rounded-lg border border-gray-200'>
            ...
            <div className='flex flex-col gap-4'>
              ...
              <div className='inline-flex items-center justify-between p-3 rounded-lg border border-gray-200'>
                ...
                <Switch
                  defaultSelected={data.TSX_WATCH ?? false}
                  {...methods.register('TSX_WATCH')}
                  color='success'
                />
              </div>
            </div>
