 ...
 * @param isWatch 是否监听文件变化并重启
 */
export const runTsx = async (pkg, isWatch = false) => {
    setMode('dev');
    setLang('ts');
    const list = ['watch'];
    if (pkg?.karin?.include) {
        pkg.karin.include.forEach((item) => list.push(`--include "${item}"`));
    }
    if (pkg?.karin?.exclude) {
        pkg.karin.exclude.forEach((item) => list.push(`--exclude "${item}"`));
    }
