 ...
 */
export const checkPkgUpdate = async (name) => {
 ...
 * @param name 包名
 */
export const getPkgVersion = async (name) => {
 ...
 */
export const getRemotePkgVersion = async (name, tag = 'latest') => {
 ...
 */
export const updatePkg = async (name, tag = 'latest') => {
    const logger = global?.logger || console;
    try {
        const local = await getPkgVersion(name);
 ...
 */
export const updateAllPkg = async () => {
    const logger = global?.logger || console;
    try {
 ...
 */
export const checkGitPluginUpdate = async (filePath, time = 120) => {
    const logger = global?.logger || console;
    try {
        /** 检查一下路径是否存在 */
        if (!fs.existsSync(filePath))
            return { status: 'error', data: new Error('路径不存在') };
        /** 检查是否有.git文件夹 */
        if (!fs.existsSync(`${filePath}/.git`))
            return { status: 'error', data: new Error('该路径不是一个git仓库') };
        /** 设置超时时间 */
 ...
 */
export const getHash = async (filePath, short = true) => {
 ...
 * ```
 */
export const getTime = async (filePath) => {
    const cmd = 'git log -1 --format=%cd --date=format:"%Y-%m-%d %H:%M:%S"';
    const { stdout, error } = await exec(cmd, { cwd: filePath });
    if (error)
        return '获取最后一次提交时间失败，请重试或更新Git~';
    return stdout;
};
/**
 ...
 * @param time 任务执行超时时间 默认120s
 */
export const updateGitPlugin = async (filePath, cmd = 'git pull', time = 120) => {
    const logger = global?.logger || console;
    try {
        /** 检查一下路径是否存在 */
        if (!fs.existsSync(filePath))
 ...
 * @param time 任务执行超时时间 默认120s
 */
export const updateAllGitPlugin = async (cmd = 'git pull', time = 120) => {
    try {
        const logger = global?.logger || console;
