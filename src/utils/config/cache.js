 ...
 * @param selfId 机器人ID
 */
export const getGroupOrGuildCache = (groupOrGuildId, selfIdOrChannelId, selfId) => {
    let keys;
    if (selfId) {
        keys = [
 ...
 * @param config 配置
 */
export const initGroupOrGuildCfg = (config) => {
 ...
 * @param key 键
 */
export const addValue = (data, key) => {
 ...
 * @param data 配置
 */
export const setGroupOrGuildCache = (keys, data) => {
    for (const key of keys) {
        if (cache.groupGuild[key]) {
            if (!data[key])
                continue;
 ...
 * @param config 配置
 */
export const initFriendDirectCfg = (config) => {
    config.alias = setStr(config.alias);
    return config;
};
/**
 ...
 * @param selfId 机器人ID
 */
export const getFriendOrDirectCache = (userId, selfId) => {
    let keys;
    if (selfId) {
 ...
 * @param data 配置
 */
export const setFriendOrDirectCache = (keys, data) => {
    for (const key of keys) {
        if (cache.friendDirect[key]) {
            if (!data[key])
                continue;
