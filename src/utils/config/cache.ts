 ...
 * @param config 配置
 */
export const initGroupOrGuildCfg = (config: GroupGuild) => {
 ...
 * @param config 配置
 */
export const initFriendDirectCfg = (config: FriendDirect) => {
  config.alias = setStr(config.alias)
  return config
}
 ...
 * @param data 配置
 */
export const setFriendOrDirectCache = (keys: string[], data: Record<string, FriendDirect>) => {
  for (const key of keys) {
    if (cache.friendDirect[key]) {
      if (!data[key]) continue
