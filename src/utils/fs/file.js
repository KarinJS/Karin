 ...
 * @returns 标准化后的路径
 */
export const absPath = (file, absPath = true, prefix = false) => {
    file = file.replace(/\\/g, '/');
 ...
 */
export const createPluginDir = async (name, files) => {
 ...
 * @returns 符合条件的文件列表
 */
export const getFiles = (filePath, suffixs = []) => {
    if (!fs.existsSync(filePath)) {
 ...
 */
export const copyFilesSync = (files, defaulPath, userPath) => {
    files.forEach(file => {
 ...
 * @param userPath 用户配置文件路径
 */
export const copyFiles = async (files, defaulPath, userPath) => {
 ...
 * ```
 */
export const copyConfigSync = (defaulPath, userPath, suffixs = [], isThrow = false) => {
    try {
 ...
 * ```
 */
export const copyConfig = async (defaulPath, userPath, suffixs = [], isThrow = false) => {
    try {
        const files = getFiles(defaulPath, suffixs);
        await Promise.all([
