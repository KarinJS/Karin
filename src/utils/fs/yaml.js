...
export class YamlEditor {
     ...
     * @param isSplit - 是否使用分割路径路径，默认为 `true`
     */
    comment(path, comment, prepend = true, isSplit = true) {
        if (!path)
 ...
 * @param options 注释配置
 */
export const save = (path, value, options) => {
    if (!options) {
        fs.writeFileSync(path, YAML.stringify(value));
        return;
    }
    ...
    const comment = typeof options === 'string'
        ? JSON.parse(fs.readFileSync(options, 'utf8'))
        : options;
 ...
 */
export const comment = (filePath, commentConfig) => {
    const editor = new YamlEditor(filePath);
    const comment = typeof commentConfig === 'string'
        ? JSON.parse(fs.readFileSync(commentConfig, 'utf8'))
        : commentConfig;
    applyComments(editor, comment);
    editor.save();
};
 ...
 */
export const applyComments = (editor, comments) => {
