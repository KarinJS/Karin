...
import { initAlias, initEmit, initMsg, initRole, log, groupPrint, groupFilterEvent, guildPrint, disableViaAdapter, disableViaPluginWhitelist, disableViaPluginBlacklist, } from '../other/handler';
...
export const groupHandler = async (ctx) => {
    ...
    // TODO: 中间件实现
    if (filter) {
        groupsDeal(ctx, group, isPrint, (plugin) => {
            if (plugin.event !== 'message' && plugin.event !== 'message.group')
                return false;
            if (![
                'all',
                'master',
 ...
 * @param ctx 群聊临时消息事件
 */
export const groupTempHandler = async (ctx) => {
    /** 基本配置 */
    const config = cfg();
    /** 好友配置 */
    const group = getGroupCfg(ctx.groupId, ctx.selfId);
    ...
    if (filter) {
        groupsDeal(ctx, group, isPrint, (plugin) => {
            if (plugin.event !== 'message' && plugin.event !== 'message.groupTemp')
                return false;
 ...
 * @param ctx 频道消息事件
 */
export const guildHandler = async (ctx) => {
    /** 基本配置 */
    const config = cfg();
    /** 好友配置 */
    ...
    if (filter) {
        groupsDeal(ctx, group, isPrint, (plugin) => {
            if (plugin.event !== 'message' && plugin.event !== 'message.guild')
                return false;
            if (![
                'all',
                'master',
 ...
 * @param filter 场景过滤器
 */
const groupsDeal = async (ctx, config, isPrint, filter) => {
 ...
 * @param isPrint 是否打印日志
 */
const groupsCmd = async (ctx, plugin, config, isPrint) => {
    ...
    try {
        if (!Permission.groups(ctx, plugin))
            return false;
        /** 是否继续匹配下一个插件 */
        let next = false;
        if (plugin.type === 'fnc') {
