...
import { log, disableViaAdapter, disableViaPluginWhitelist, disableViaPluginBlacklist, } from './handler';
/**
 * @description 初始化提示信息
 * @param ctx 事件对象
 */
export const initTips = (ctx) => {
    switch (ctx.subEvent) {
        case 'friendPoke':
 ...
 * @param level 日志等级
 */
export const initPrint = (ctx, type, prefix, level = 'info') => {
 ...
 * @param filter 场景过滤器
 */
export const deal = async (ctx, config) => {
...
const fnc = async (ctx, plugin, config) => {
