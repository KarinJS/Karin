 ...
 * @param text 日志内容
 */
export const log = (userId, text) => logger.debug(`[消息过滤][${userId}] ${text}`);
/**
 * @description 初始化`msg` `rawMessage`
 * @param ctx 消息事件对象
 */
export const initMsg = (ctx) => {
 ...
 * @param config 基本配置
 */
export const initRole = (ctx, config) => {
    /** 主人 */
 ...
 * @param alias Bot前缀列表
 */
export const initAlias = (ctx, alias) => {
 ...
 * @returns `true` 表示通过
 */
export const disableViaAdapter = (plugin, protocol) => {
    /** 插件指定只允许特定适配器使用 */
 ...
 * @returns `true` 表示通过
 */
export const disableViaPluginWhitelist = (plugin, config) => {
    /** 未设置白名单 */
    if (!config.enable.length)
        return true;
    /**
     * 编写规则:
 ...
 * @returns `true` 表示通过
 */
export const disableViaPluginBlacklist = (plugin, config) => {
    /** 未设置黑名单 */
 ...
 * @returns `true` 表示通过
 */
export const privateFilterEvent = (ctx, config, friend, cd) => {
    if (!cd) {
 ...
 * @returns `true` 表示通过
 */
export const groupFilterEvent = (ctx, config, group, cd) => {
    if (!cd) {
        log(ctx.userId, `当前处于CD中: ${ctx.eventId}`);
        return false;
    }
 ...
 * @returns `true` 表示通过
 */
export const groupPrint = (ctx, config) => {
 ...
 * @returns `true` 表示通过
 */
export const guildPrint = (ctx, config) => {
