...
export class Permission {
     ...
     */
    static handleNoPermission(ctx, authFailMsg, defaultMsg) {
    ...
    static groups(ctx, plugin) {
        ...
        if (ctx.isGroup) {
            if (plugin.permission === 'group.owner') {
                if (ctx.isMaster || ctx.isAdmin || ctx.sender?.role === 'owner')
                    return true;
