...
export class WsAdapterOneBot11 extends AdapterOneBot {
    /** 请求id */
    seq;
    /** WebSocket实例 */
    socket;
     ...
     */
    async init(selfId, url, communication) {
    ...
    onEvent() {
        this.socket.on('message', (rawData) => {
            ...
            if (json.echo) {
                ...
                if (data.post_type === "meta_event" /* OB11Event.MetaEvent */ && data.meta_event_type === 'heartbeat') {
    ...
    async setAdapterInfo() {
        const info = await this.sendApi("get_version_info" /* Action.getVersionInfo */, {});
        this.adapter.name = info.app_name;
        this.adapter.version = info.app_version;
    }
    /** 设置登录号详细信息 */
    async setBotInfo() {
        const info = await this.sendApi("get_login_info" /* Action.getLoginInfo */, {});
