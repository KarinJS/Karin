...
export class AdapterOneBot extends AdapterBase {
    ...
    eventHandlers(data, str) {
        if (data.post_type === "message" /* OB11Event.Message */) {
            createMessage(data, this);
            return;
        }
        if (data.post_type === "notice" /* OB11Event.Notice */) {
            createNotice(data, this);
            return;
        }
        if (data.post_type === "request" /* OB11Event.Request */) {
            createRequest(data, this);
            return;
        }
        if (data.post_type === "meta_event" /* OB11Event.MetaEvent */) {
    ...
    async sendMsg(contact, elements, retryCount) {
        try {
            ...
            let res;
            if (scene === 'group') {
                res = await this.sendApi("send_msg" /* Action.sendMsg */, {
                    message_type: 'group',
     ...
     */
    async SendMessage(contact, elements, retryCount) {
    ...
    async sendLongMsg(contact, resId) {
        ...
        const { scene, peer } = contact;
        if (scene === 'group') {
            result = await this.sendApi("send_group_msg" /* Action.sendGroupMsg */, {
                group_id: Number(peer),
                message: [{ type: 'forward', data: { id: resId } }],
            });
        }
        else {
            result = await this.sendApi("send_private_msg" /* Action.sendPrivateMsg */, {
                user_id: Number(peer),
                message: [{ type: 'forward', data: { id: resId } }],
            });
        }
    ...
    async recallMsg(contact, messageId) {
        try {
            await this.sendApi("delete_msg" /* Action.deleteMsg */, { message_id: Number(messageId) });
            return true;
        }
        catch {
            return false;
    ...
    async getMsg(contact, messageId) {
        const result = await this.sendApi("get_msg" /* Action.getMsg */, { message_id: Number(messageId) });
     ...
     * @returns 包含历史消息的数组
     */
    async getHistoryMsg(contact, startMsgId, count) {
        let options;
        ...
        if (contact.scene === 'group') {
            ...
            res = await this.sendApi("get_group_msg_history" /* Action.getGroupMsgHistory */, options);
        }
        else {
            options.user_id = Number(contact.peer);
            res = await this.sendApi("get_friend_msg_history" /* Action.getFriendMsgHistory */, options);
        }
        const all = [];
     ...
     */
    async sendLike(targetId, count) {
        try {
            await this.sendApi("send_like" /* Action.sendLike */, { user_id: Number(targetId), times: count });
            return true;
        }
        catch {
            return false;
    ...
    async groupKickMember(groupId, targetId, rejectAddRequest, kickReason) {
        try {
            await this.sendApi("set_group_kick" /* Action.setGroupKick */, {
    ...
    async setGroupMute(groupId, targetId, duration) {
        try {
            await this.sendApi("set_group_ban" /* Action.setGroupBan */, { group_id: Number(groupId), user_id: Number(targetId), duration });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupAllMute(groupId, isBan) {
        try {
            await this.sendApi("set_group_whole_ban" /* Action.setGroupWholeBan */, { group_id: Number(groupId), enable: isBan });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupAdmin(groupId, targetId, isAdmin) {
        try {
            await this.sendApi("set_group_admin" /* Action.setGroupAdmin */, { group_id: Number(groupId), user_id: Number(targetId), enable: isAdmin });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupMemberCard(groupId, targetId, card) {
        try {
            await this.sendApi("set_group_card" /* Action.setGroupCard */, { group_id: Number(groupId), user_id: Number(targetId), card });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupName(groupId, groupName) {
        try {
            await this.sendApi("set_group_name" /* Action.setGroupName */, { group_id: Number(groupId), group_name: groupName });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupQuit(groupId, isDismiss) {
        try {
            await this.sendApi("set_group_leave" /* Action.setGroupLeave */, { group_id: Number(groupId), is_dismiss: isDismiss });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupMemberTitle(groupId, targetId, title) {
        try {
            await this.sendApi("set_group_special_title" /* Action.setGroupSpecialTitle */, { group_id: Number(groupId), user_id: Number(targetId), special_title: title });
            return true;
        }
        catch {
            return false;
    ...
    async GetCurrentAccount() {
        const res = await this.sendApi("get_login_info" /* Action.getLoginInfo */, {});
        return {
    ...
    async getStrangerInfo(targetId) {
        const result = await this.sendApi("get_stranger_info" /* Action.getStrangerInfo */, { user_id: Number(targetId[0]), no_cache: true });
        return {
            ...result,
            userId: targetId,
            user_id: targetId,
    ...
    async getFriendList(refresh) {
        const friendList = await this.sendApi("get_friend_list" /* Action.getFriendList */, {});
    ...
    async getGroupInfo(groupId, noCache) {
        const info = await this.sendApi("get_group_info" /* Action.getGroupInfo */, { group_id: Number(groupId), no_cache: noCache });
    ...
    async getGroupList(refresh) {
        const groupList = await this.sendApi("get_group_list" /* Action.getGroupList */, { no_cache: refresh });
    ...
    async getGroupMemberInfo(groupId, targetId, refresh) {
        ...
        const info = await this.sendApi("get_group_member_info" /* Action.getGroupMemberInfo */, { group_id: Number(groupId), user_id: userId, no_cache: refresh });
        return {
            ...info,
            userId: targetId,
            uid: targetId,
            ...
            sex: info.sex,
            get sender() {
                return senderGroup(this.userId, this.role, this.nick, this.sex, this.age, this.card, this.area, this.level, this.title);
            },
        };
    }
    /**
    ...
    async getGroupMemberList(groupId, refresh) {
        const list = await this.sendApi("get_group_member_list" /* Action.getGroupMemberList */, { group_id: Number(groupId), no_cache: refresh });
        return list.map(v => {
            ...
            return {
                ...
                sex: v.sex,
                get sender() {
                    return senderGroup(targetId, v.role, v.nickname, v.sex, v.age, v.card, v.area, Number(v.level), v.title);
                },
            };
        });
    }
    ...
    async getGroupHonor(groupId) {
        const groupHonor = await this.sendApi("get_group_honor_info" /* Action.getGroupHonorInfo */, { group_id: Number(groupId), type: 'all' });
        const result = [];
    ...
    async setMsgReaction(contact, messageId, faceId, isSet) {
        try {
            await this.sendApi("set_msg_emoji_like" /* Action.setMsgEmojiLike */, { message_id: messageId, emoji_id: faceId, is_set: isSet });
            return true;
        }
        catch {
            return false;
     ...
     */
    async GetVersion() {
        const res = await this.sendApi("get_version_info" /* Action.getVersionInfo */, {});
        return {
            name: res.app_name,
    ...
    async getGroupHighlights(groupId, page, pageSize) {
        const list = [];
        const res = await this.sendApi("get_essence_msg_list" /* Action.getEssenceMsgList */, { group_id: Number(groupId) });
        for (const v of res) {
     ...
     * @returns 此接口的返回值不值得信任
     */
    async uploadFile(contact, file, name, folder) {
        try {
            if (contact.scene === 'group') {
                await this.sendApi("upload_group_file" /* Action.uploadGroupFile */, { group_id: Number(contact.peer), file, name, folder });
            }
            else {
                await this.sendApi("upload_private_file" /* Action.uploadPrivateFile */, { user_id: Number(contact.peer), file, name });
            }
            return true;
        }
        catch {
     ...
     */
    async setGgroupHighlights(groupId, messageId, create) {
        try {
            await this.sendApi(create ? "set_essence_msg" /* Action.setEssenceMsg */ : "delete_essence_msg" /* Action.deleteEssenceMsg */, { message_id: Number(messageId) });
            return true;
        }
        catch {
            return false;
    ...
    async setFriendApplyResult(requestId, isApprove, remark) {
        try {
            await this.sendApi("set_friend_add_request" /* Action.setFriendAddRequest */, { flag: requestId, approve: isApprove, remark });
            return true;
        }
        catch {
            return false;
    ...
    async setGroupApplyResult(requestId, isApprove, denyReason) {
        try {
            await this.sendApi("set_group_add_request" /* Action.setGroupAddRequest */, { flag: requestId, approve: isApprove, sub_type: 'add', reason: denyReason });
            return true;
        }
        catch {
            return false;
    ...
    async setInvitedJoinGroupResult(requestId, isApprove) {
        try {
            await this.sendApi("set_group_add_request" /* Action.setGroupAddRequest */, { flag: requestId, approve: isApprove, sub_type: 'invite' });
            return true;
        }
        catch {
            return false;
     ...
     * @returns 适配器消息元素
     */
    forwardKarinConvertAdapter(elements) {
        const messages = [];
        for (const elem of elements) {
            if (elem.subType === 'messageID') {
                ...
                if (typeof elem.options === 'object') {
                    if (elem.options.prompt)
                        node.data.prompt = elem.options.prompt;
                    if (elem.options.summary)
                        node.data.summary = elem.options.summary;
                    if (elem.options.source)
                        node.data.source = elem.options.source;
                }
                messages.push(node);
            }
        }
    ...
    async sendForwardMsg(contact, elements, options) {
        if (contact.scene === 'group') {
            const result = await this.sendApi("send_group_forward_msg" /* Action.sendGroupForwardMsg */, {
        ...
        if (contact.scene === 'friend') {
            const result = await this.sendApi("send_private_forward_msg" /* Action.sendPrivateForwardMsg */, {
