 ...
 * @param error 错误信息
 */
export const buildError = (selfId, action, request, error) => {
    if (error) {
 ...
 * @param file 文件路径
 */
export const fileToBase64 = (file, url) => {
    if (!url || !file.startsWith('file://'))
        return file;
...
/**
   ...
   */
export const KarinConvertAdapter = (data, onebot) => {
    const elements = [];
    for (const i of data) {
        const type = i.type;
        switch (i.type) {
