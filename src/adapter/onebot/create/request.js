...
import { createGroupApplyRequest, createGroupInviteRequest, createPrivateApplyRequest, } from '@/event/create';
/**
 * 创建请求事件
...
export const createRequest = (event, bot) => {
    ...
    // 新好友申请
    if (event.request_type === "friend" /* OB11RequestType.Friend */) {
    ...
    // 新成员加群申请
    if (event.request_type === "group" /* OB11RequestType.Group */ && event.sub_type === 'add') {
    ...
    // 邀请Bot加入群聊
    if (event.request_type === "group" /* OB11RequestType.Group */ && event.sub_type === 'invite') {
        const groupId = event.group_id + '';
        const contact = contactGroup(groupId);
