...
describe('path/index', () => {
  ...
  it('findFiles catch returns [] on missing dir (index.ts)', async () => {
    const missing = path.join(os.tmpdir(), `missing-${Date.now()}`)
    const { findFiles } = await import('./index')
    const res = await findFiles(missing)
    expect(res).toEqual([])
  })

  it('findFiles string/array ext and rel/recursive (index.ts)', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fi-'))
    fs.writeFileSync(path.join(dir, 'a.ts'), 'x')
    fs.writeFileSync(path.join(dir, 'b.js'), 'x')
    const sub = path.join(dir, 'sub')
    fs.mkdirSync(sub)
    fs.writeFileSync(path.join(sub, 'c.js'), 'x')
    const { findFiles } = await import('./index')
    const onlyJs = await findFiles(dir, { ext: 'js' })
    expect(onlyJs).toEqual(['b.js'])
    const relJs = await findFiles(dir, { ext: ['js'], pathsType: 'rel' })
    expect(relJs.some(p => p.includes('b.js'))).toBe(true)
    const recursiveAbs = await findFiles(dir, { ext: 'js', recursive: true, pathsType: 'name' })
    expect(recursiveAbs.every(p => path.isAbsolute(p))).toBe(true)
    fs.rmSync(dir, { recursive: true, force: true })
  })

  it('isSubPath resolves to abs and returns boolean', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'root-'))
    const inside = path.join(root, 'a', 'b')
    fs.mkdirSync(inside, { recursive: true })
    fs.writeFileSync(path.join(inside, 'x.txt'), 'x')
    const { isSubPath } = await import('./index')
    expect(isSubPath(root, inside)).toBe(true)
    const outside = path.join(os.tmpdir(), `other-${Date.now()}`)
    expect(isSubPath(root, outside)).toBe(false)
    fs.rmSync(root, { recursive: true, force: true })
  })

  it('isPathEqual non-win branch returns false', async () => {
    const original = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', { value: 'linux' })
    const { isPathEqual } = await import('./index')
    expect(isPathEqual('/a', '/b')).toBe(false)
